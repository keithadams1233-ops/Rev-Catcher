...
**Phases 1–10 are implemented**: Next.js + Supabase wiring, the full database
...
same real transaction data), the ROI report (`src/lib/roi/` — real
...
before/after values instead of a projected estimate), and pilot
hardening (an inactive `locations.active` gates CSV import/leak
detection/challenge launch for real now; a manager can cancel a pending
reward redemption and get the points reversed; a 100%-duplicate
re-upload no longer shows as a failed import).
...
Seven flows *are* real writes, not mockups: the goal builder launches an
...
challenges/team goals; the gamification engine actually reads today's
transactions to progress missions, award XP/points, level employees up,
advance streaks, and unlock badges; and cancelling a pending reward
redemption actually reverses the point debit through a new ledger row,
not a silent delete.

Phase 10 (pilot hardening) found and closed two real gaps rather than
adding a new engine: `locations.active` (present since Phase 1, never
enforced) now actually blocks new imports/leaks/challenges for a closed
location, and a manager can now undo a reward redemption before it's
fulfilled. Everything else on the spec's hardening checklist — corrupted
CSV, refunds/voids, missing employee IDs, small samples, challenge
cancellation, RLS/Server Action authorization, mobile layouts, empty
states — was audited and confirmed already solid from earlier phases'
own discipline; see `ARCHITECTURE.md`'s "Pilot hardening" section for
what was checked.
...
src/
  app/
    ...
    manager/           Rev Catcher screens (role-gated) — home, leaks (+ leaks/[id]
                         ...
                         with a manual Update Progress trigger, cancel action, and
                         a completed challenge's ROI report + new builder),
                         people (roster + pending-redemptions cancel/refund
                         action), settings, settings/data-sources (CSV import
                         wizard + history)
